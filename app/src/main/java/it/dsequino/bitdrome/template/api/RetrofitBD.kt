...
class RetrofitBD {
    //region VARIABLES
    private lateinit var onPostLogin: OnPostLogin
...
//endregion
    ...
    fun postLogin(email: String, password: String) {
        val lngApi = getClient().create(BDApi::class.java)
        ...
        call.enqueue(object : ResponseHandler<LoginResponse>() {
            override fun onResponse(response: LoginResponse?) {
                onPostLogin.onPostLogin(response!!)
            }

            override fun onError(error: Throwable) {
                onPostLogin.onError(error)
            }

        })
    }
    interface OnPostLogin : OnErrorListener {
        fun onPostLogin(loginResponse: LoginResponse)
    }
    fun setOnPostLoginListener(onPostLogin: OnPostLogin) {
        this.onPostLogin = onPostLogin
    }
    //endregion
}